...
class AlphaMinerplus:
                ...
                for trace in traces_without_duplicates:
                    if all_relations == None:
                        ...
                        if trace.find(two_element_transitions) >= 0:
                            if all_relations == "<-":
                                if (
                                    trace.find(three_element_transition_1) <= 0
    ...
    def get_pairs(self):
        ...
        while i < j:
            ...
            for pair in choice_pairs:
                ...
                if (
                    len(SortedSet(current_choice_pair).intersection(SortedSet(pair)))
                    != 0
                ):
                    for e1 in pair:
                        if union == False:
                        ...
                        for e2 in current_choice_pair:
                            if (
                                self.relations[e1][e2] != "#"
                            ):  # check if the activities are independent
                                union = False
                                break
                    if (
                        union
                    ):  # if not independent, create a new pair which is the union of the 2 activities eg  [a,d]  +  [a, f] -> [a, d, f]
            ...
            for pair_choices2 in choice_pairs:
                ...
                for activity1 in pair_choices1:
                    ...
                    for activity2 in pair_choices2:
                        ...
                        if (
                            relation_between_pair is not None
        ...
        for pair1 in self.pairs:
            ...
            for pair2 in self.pairs:
                if pos1 != pos2:
                    ...
                    if SortedSet(flat_pair1).issubset(flat_pair2) and SortedSet(
                        flat_pair1
                    ) != SortedSet(flat_pair2):
                        append_flag = False
                pos2 = pos2 + 1
    ...
    def remove_length_one_loops(self) -> SortedSet:
        ...
        self.log_without_length_one_loops = self.transitions.difference(
            self.length_one_loops
        )
